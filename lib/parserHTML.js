...
module.exports = {
			...
			if(code[0].substr(0,1) != '/'){
				...
				if(code[0] === 'script' || isScript || (code[0] == 'script' && tagmatch[0].substr(-1) === '/')){
					...
					if(attributes){
						...
						view.dom[key].treenode = attributes.treenode;
					}
					if(tagmatch[0].substr(-1) === '/')
						treenode--;
				}
	...
	getChildren: function(tagsarray,tag,attributes,linenumber,treenode){
		...
		_.each(tagsarray, function(looptag){
			...
			var getattrs;
            _.each(innertagname, function(childattributes, key){
                if(key != 0 && childattributes != '/')
                    getattrs = getattrs ? getattrs + ' ' + childattributes : childattributes;
            })
			loopline++;
				...
				if((!code && loopnode === treenode) && (closingtag == innertagname[0]) ||  tag == innertagname[0] && innertag[0].substr(-1) == '/'){
					...
					if(loopline > linenumber){
						...
						if(loopnode == treenode && !siblingsdone){
							var sibtag = {};
							sibtag.tag = innertagname[0];
							sibtag.attributes = getattrs;
                            sibtag.treenode = treenode;
							siblingsarray.push(sibtag);
						}
						else if(loopnode > treenode && !parenttagclosed){
							var childtag = {};
							childtag.tag = innertagname[0];
							childtag.attributes = getattrs;
                            childtag.treenode = loopnode;
							childrenarray.push(childtag);
						}
					}
				}
			}
